...
const addHotels = asyncHandler(async (req, res) => {
  ...
  if (!name || !description || !place || !lat || !lon || !address || !pricePerNight) {
  ...
  const hotelpath = req.file?.path;
  if (!hotelpath) {
    throw new ApiError(400, "Invalid hotel image path");
  }

  const hotelobj = await uploadoncloudinary(hotelpath);
  if (!hotelobj.url) {
    throw new ApiError(400, "Error uploading hotel to Cloudinary");
  }

  const hotel = await Hotel.create({
    imageUrl: hotelobj.url,
    name,
    description,
    place,
  ...
  console.log("hotelscachedb before",hotel)
  // hotelcachedb.set(place, hotel);
