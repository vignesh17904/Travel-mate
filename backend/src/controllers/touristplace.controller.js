...
const gettouristplaces = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (tour.length === 0) {
      throw new ApiError(404, "No tourist places found for this place");
    }
    ...
    const touritems = await Promise.all(
      tour
        ...
        .map(async (item) => ({
          ...
          imageurl:
            (await fetchphoto(`tourist place ${item.properties.name} in ${item.properties.city}`)) || null,
          description:
            (await geminiresponse({
              placename: item.properties.name,
              question: "Tell me about this tourist place?",
              words: 20,
            })) || null,
          location: {
    ...
    return res.status(200).json(new ApiResponse(200, { touritems }, "Tourist places fetched successfully"));
  } catch (error) {
