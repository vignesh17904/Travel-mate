    ...
    <React.Fragment>
      ...
      <nav className="w-full absolute top-0 p-5 flex justify-between text-white z-10">
        ...
        <div className="flex items-center space-x-2 mt-5">
          ...
          <input className="bg-inherit border-b outline-0 border-gray-300 p-1 w-full" type="text" placeholder="search" />
        </div>
      </nav>
      ...
      <div className="absolute top-0 h-screen flex flex-col space-y-10 justify-center items-center bg-black opacity-75 xl:w-1/3 sm:w-1/2 z-0">
        ...
        <div className="space-x-4">
          {user ? (
            ...
            <>
              <button onClick={() => navigate('/SignIn')} className="bg-gray-300 py-2 px-5 rounded-md hover:bg-white">Sign in</button>
              <button onClick={() => navigate('/SignUp')} className="text-gray-300 border rounded-md py-2 px-5 hover:bg-white hover:text-black">Sign Up</button>
            </>
          )}
        </div>
