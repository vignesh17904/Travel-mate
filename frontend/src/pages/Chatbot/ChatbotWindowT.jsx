...
import { GeminiResponseT } from "../../utils/GeminiResponse.js";
import { useParams } from "react-router";

export default function ChatbotWindowT({ onClose }) {
  ...
  useEffect(() => {
    setMessages([
      {
        from: "bot",
        text: `Hi! I’m TravelBuddy 🤖 here to help you. Ask anything about travel plans, transport, or places in ${CityName}.`,
      },
    ]);
  }, [CityName]);
  ...
  const handleAsk = async () => {
    ...
    const reply = await GeminiResponseT({
      placename: CityName,
      question: question,
    });

   const formattedParas = reply
  .split(/\n\s*\n|(?<=[.!?])\s+(?=[A-Z])/g) 
  .map(p => p.trim())
  .filter(p => p.length > 0);

formattedParas.forEach(p => {
  setMessages(prev => [...prev, { from: "bot", text: p }]);
});
  };
    ...
    <div className="flex flex-col h-full w-full bg-white/80 backdrop-blur-md p-4 rounded-lg shadow-xl border border-gray-300">
      ...
      <div className="flex items-center border rounded-full px-3 py-2 shadow-sm bg-white">
        <input
          ...
          placeholder="Ask about travel, transport, places..."
