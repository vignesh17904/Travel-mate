...
import { MdSmartToy } from "react-icons/md"; // cooler AI bot icon
import ChatbotWindow from "./ChatbotWindow";

export default function ChatbotLauncher({ hotels }) {
  ...
  return (
    <>
      ...
      >
        ...
        <span className="text-xs font-medium leading-tight">Need help? <br /> Ask TravelBuddy</span>
      </button>

      {isOpen && (
        <div className="fixed bottom-24 right-6 w-96 h-[500px] bg-white border rounded-xl shadow-xl z-50 overflow-hidden">
          <ChatbotWindow onClose={() => setIsOpen(false)} hotels={hotels} />
        </div>
      )}
    </>
  );
